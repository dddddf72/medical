...
class PercentProgressBar(QWidget):
    ...
    ShowFont = True  # 是否显示文字
    ShowFreeArea = False  # 显示背后剩余
    ShowSmallCircle = False  # 显示带头的小圆圈
    TextColor = QColor(0, 191, 255)  # 文字颜色
    BorderColor = QColor(0, 191, 255)  # 边框圆圈颜色
    BackgroundColor = QColor(Qt.transparent)  # 背景颜色
    FontSize = QFont('Arial', 18)
    ...
    def showFont(self) -> bool:
        return self.ShowFont

    @showPercent.setter
    def showFont(self, showFont: bool):
        if self.ShowFont != showFont:
            self.ShowFont = showFont
            self.update()
